...
## 이미 리젝당했다면? 메일을 붙여넣으세요

애플 리젝 메일(`ITMS-91053` …)은 암호문 같습니다. `explain`은 이를 해독합니다 —
**안정적인 카테고리 코드만** 사용하고 메일 문구는 절대 파싱하지 않으며,
프로젝트를 주면 원인 패키지와 수정법까지 짚어줍니다:

```bash
npx sdk-privacy-scan explain "…ITMS-91053: Missing API declaration …
NSPrivacyAccessedAPICategoryFileTimestamp…" --project . --lang ko
```
...
NSPrivacyAccessedAPICategoryFileTimestamp (File timestamps)
  이 프로젝트에서 유력한 원인:
    ✗ react-native-fs — 커버하는 배포 매니페스트가 없습니다. 해결: 패키지를
      업데이트하거나, PrivacyInfo.xcprivacy에 …FileTimestamp (C617.1)를 선언하세요.
  승인된 사유 코드:
    C617.1  앱 컨테이너 내부 파일의 타임스탬프 (일반적인 경우)
```

파이프도 됩니다: `pbpaste | npx sdk-privacy-scan explain -p .`

## 한국어 출력

```bash
npx sdk-privacy-scan ./my-app --lang ko    # 모든 안내 문구가 한국어로 나옵니다
```

기본은 영어입니다. 기술 식별자(`NSPrivacy…`, 사유 코드)는 애플·구글 문서와
항상 일치하도록 두 언어 모두에서 영어로 유지됩니다.
